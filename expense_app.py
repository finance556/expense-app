...
XERO_SCOPES        = "accounting.transactions accounting.contacts openid profile email offline_access"
...
{% else %}
<form id="expForm" method="POST" action="/submit" novalidate>
  ...
  <div class="card" id="desktop-table" style="display:none">
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Description *</th>
            <th style="width:115px;text-align:right">Amount (AUD) *</th>
...
function addLine(d, a, s, desc, amt) {
  ...
  card.innerHTML = `
    ...
    <div class="field">
      <label>Account *</label>
      <select name="a${i}" required>${accountOptions(a||'')}</select>
    </div>
    <div class="field">
...
function calcTotal() {
  ...
  const fmt = '$' + t.toLocaleString('en-AU', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
def submit():
    ...
    for i in indices:
        ...
        try:
            ...
            continue
        if not (date and account and amount > 0):
            continue
